...
class userModel extends model
{
     ...
     * @param $userid用户id号
     */
    public function getAnnexByAccount($account,$pager=null){
        $annex =  $this->dao->select("name,num,case locked when 0 then '可享用' ELSE '已关闭' end as locked")
           ->from(TABLE_ANNEX)->alias('a')
            ->leftJoin(TABLE_USERANNEX)->alias('u')
             ->on('a.id=u.annexid')->
             where('u.account')->eq($account)->page($pager)
            ->query();
        return $annex;
    }
